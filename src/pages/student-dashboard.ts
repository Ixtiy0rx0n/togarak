...
async function bootstrap(): Promise<void> {
    ...
    byId<HTMLFormElement>("loginForm").addEventListener("submit", async (event) => {
        ...
        const result = await login(byId<HTMLInputElement>("username").value.trim(), byId<HTMLInputElement>("password").value.trim(), "student");
        if (!result) {
    ...
    document.querySelectorAll<HTMLElement>('[data-action="show-groups"]').forEach((button) => {
    ...
    byId<HTMLFormElement>("submitForm").addEventListener("submit", async (event) => {
        event.preventDefault();
        const file = byId<HTMLInputElement>("s_file").files?.[0];
        if (!file) {
        ...
        await submitAssignment({
            assignmentId: Number(byId<HTMLInputElement>("s_assignment_id").value),
            studentId: currentStudentId,
            fileName: file.name,
            fileDataUrl: await readFileAsDataUrl(file)
        });
        byId("submitModal").classList.add("hidden");
        byId<HTMLFormElement>("submitForm").reset();
    ...
    document.addEventListener("click", async (event) => {
        ...
        if (target.dataset.submitAssignment) {
            byId<HTMLInputElement>("s_assignment_id").value = target.dataset.submitAssignment;
